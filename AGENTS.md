...
- **Two top-level skills:**
  - [`agent/skills/general/SKILL.md`](agent/skills/general/SKILL.md) — default front-facing skill. Handles greetings and ambiguous prompts; calls `route_to_skill("sow-response")` when SOW/RFP intent is detected so the desktop client can auto-trigger the next turn against the workflow skill.
...
- **Routing tools** (in [`runtime/state_tools.py`](agent/src/charter_agent/runtime/state_tools.py)): `route_to_skill(skill_name)` — called by the `general` skill (or any skill) to register the active workflow skill for the project; the desktop client picks up the routing event and auto-sends the next turn against the target skill.
...
|---|---|---|
| `general` | [`agent/skills/general/SKILL.md`](agent/skills/general/SKILL.md) | Default front-facing skill. Handles greetings, ambiguity, and intent detection. Calls `route_to_skill("sow-response")` when the user's prompt looks like an SOW workflow request, so the desktop client auto-routes the next turn. |
...
**Adding a new top-level workflow.** Drop a new `agent/skills/<name>/SKILL.md`. The skill_loader picks it up at boot; foundry_host builds it a warm Agent; `_ResilientResponsesHostServer` routes to it when the preamble or `project_log["skill"]` names it. The `general` skill should learn to `route_to_skill("<name>")` for the matching intent.
...
| 3. Multi-skill scaffolding | `general` skill as front door; `route_to_skill` + `invoke_skill`; `_ResilientResponsesHostServer` per-request agent swap | ✅ done |
