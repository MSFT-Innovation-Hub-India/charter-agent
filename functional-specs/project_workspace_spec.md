...
**Skills shape the agent's behaviour declaratively, and they conform to the open [agentskills.io](https://agentskills.io/specification) specification.** Any subdirectory under `skills/` with a `SKILL.md` file is auto-loaded by the Copilot SDK at process start. Each `SKILL.md` carries YAML frontmatter with a required `name` (must match the parent directory) and `description` (must say *what* the skill does and *when* to use it, with trigger keywords for matching), plus optional `metadata`/`license`/`compatibility`/`allowed-tools`; the body is Markdown instructions for when and how to behave when the skill is "active." Each skill may also have optional `scripts/`, `references/`, and `assets/` subdirs, loaded on demand under the spec's progressive-disclosure model (discovery → activation → execution). This is how you give the agent its project-workspace specialisation — see section 7 for the skills structure recommended for this system, and [AGENTS.md §4.3](../AGENTS.md#43-agent-skills-format-agentskillsio-conformance) for the format contract enforced by `skills-ref validate` in CI. Conformance gives us portability across any agentskills.io client (Claude Code, VS Code, Goose, Gemini CLI, Kiro, fast-agent, …) for isolated skill testing without the full Foundry agent. Skills are far more transparent and auditable than dynamically generated `.py` modules; **default to skills, fall back to module generation only when project-specific code (e.g. a per-deliverable consolidator) genuinely warrants it** — see [AGENTS.md §4.4](../AGENTS.md#44-core-code-vs-skill--the-decision-rule) for the explicit decision rule between handwritten core code and skills.
...
### 5.6 Dashboard UI requirements

The dashboard is the single user-facing surface for every project. The implementation must support the following five surfaces and five interaction principles. A reviewer-facing visual mock lives at [`test-fixtures/dashboard-mock.html`](../test-fixtures/dashboard-mock.html) (open it in any browser; no build, no deps) as a reference for layout intent — **the mock is illustrative only**. The actual UI must be **fully data-driven and adapt to the project's runtime shape**: the number of owners, tasks, watch channels, exceptions, and activity entries all come from the Charter and live `state.json`, not from any fixed layout assumption. A project with three owners and one section, and a project with twelve owners across six sections, must both render correctly with no code change.

**The five required surfaces.**

1. **Owner tiles — status at a glance.** One tile per task owner (count is dynamic, sourced from `charter.stakeholders.owners` × `charter.tasks`). Each tile shows: owner name and role, the section / task they own, the file or channel the agent is watching, and a single coloured status pill — `Assigned | InProgress | At risk | Submitted` (mapping to the canonical states in [§8.3](#83-status-triangulation-truth-table) below). The pill is the **only** colour element in the tile, so the grid reads cleanly at a glance even at twenty tiles. Tile layout reflows responsively across viewport widths; on mobile widths tiles stack vertically.
2. **Signals drill-in panel.** Clicking any tile expands an **inline, non-modal** panel showing the four signals the agent triangulated for that owner — file activity, Teams thread, email reply, calendar/OOO — and the decision the agent reached (e.g. *"Marked at risk: travel auto-reply detected on the 14th; submission expected the 16th"*). The drill-in expands within the page so the coordinator does not lose context on the rest of the dashboard while comparing owners. The footer of the drill-in shows *"Refreshed on this visit, in {visitor.upn}'s context at HH:MM"* — explicit nod to the OBO identity model from [§5.5](#55-the-asymmetric-data-visibility).
3. **Activity stream — the audit log.** Reverse-chronological list sourced from `$HOME/activity.json` (every WorkIQ extraction, every classification, every drafted/executed/dismissed action, every reconciliation check, every human override). Sticky header timestamps so the latest events are visible without scrolling. This is the surface that satisfies the regulated-industry buyer; it must show *everything* the agent did, not a curated subset.
4. **Exceptions panel — what needs the coordinator now.** Lists `SuggestedAction`s with `status = pending` plus any consolidation/reconciliation flags. Colour-coded by kind: *question*, *at risk*, *reconciliation*, *amendment proposal*. **The panel does not disappear when empty** — it shows a reassurance state ("Nothing needs you right now.") so the coordinator can trust silence rather than wondering whether the panel failed to load.
5. **Consolidated deliverable progress card.** A progress bar tied to `count(submitted_tasks) / count(total_tasks)` from `state.json` — **fractional, not time-based**. A project is "done" when its sections are in, not when its deadline passes; a time-based bar would mislead. At 100% the card surfaces the link to the final consolidated artifact in SharePoint.

**The five interaction principles.**

- **Fully responsive on every visit.** No stale views, no "click to refresh." Every page load triggers an `/invocations` call (verb `render_dashboard`) that runs a capture-loop refresh cycle in the visitor's OBO context before the SPA re-renders. The cold-start "warming up…" indicator from [AGENTS.md §11.7](../AGENTS.md#117-frontend-bff-contract) is the only acceptable pre-render state.
- **The signals drill-in is non-modal.** It expands inline; the rest of the dashboard stays interactive. The coordinator must be able to open two drill-ins side-by-side on wide viewports for comparison.
- **The exceptions panel always renders.** Empty state is a first-class state ("Nothing needs you right now."), not a hidden component. Trust is built by predictable presence.
- **Activity stream is reverse-chronological with sticky timestamps.** Newest at top; date/time headers stick to the top of the scroll viewport so context is never lost while scrolling.
- **Progress is fractional, not temporal.** Driven by submission count, not elapsed time vs deadline. The deadline appears separately as a "due {date}" metadata line; it never colours or sizes the bar.

**Role-based view filtering.** All five surfaces are present for the coordinator. Owners see only their own tile expanded by default, the activity stream filtered to events that reference them, and a read-only exceptions panel. Observers see owner tiles plus the progress card; no exceptions panel, no drill-in. The filter is enforced server-side by the `render-dashboard` skill ([AGENTS.md §11.3](../AGENTS.md#113-skills-contract)) — the SPA does not hide data it received.

**Dynamic-shape contract (explicit, because it is easy to get wrong).** None of the following may be fixed in markup, CSS grid templates, or component props: number of owner tiles, number of watch channels per tile, number of activity entries, number of exceptions, number of tasks per owner. The Charter and `state.json` are the only sources of truth. Tile grids must reflow; activity and exceptions must virtualise above ~100 entries; the progress card denominator must come from `len(charter.tasks)`, never a hard-coded constant.
