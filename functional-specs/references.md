...
- **Skills.** The Copilot SDK has its own `skills/*/SKILL.md` auto-loader. We do **not** rely on it — our host runtime is MAF, and skills are loaded by `runtime/skill_loader.py` and injected into the MAF `Agent`. See [§10](#10-agent-skills-format--the-open-agentskillsio-specification).
...
**Standing rule — do not move work onto the codegen sub-agent.** Quality wishes ("Opus would phrase this nudge better") are not a reason to widen its remit. If a non-codegen step needs a different model, surface that model as a named MAF tool that the host `Agent` can select — do not invent a second reasoning surface. See [AGENTS.md invariant 12](../AGENTS.md#3-non-negotiable-architectural-invariants).
...
9. **Agent Framework + GitHub Copilot integration** — the `CopilotClient.AsAIAgent()` / `GitHubCopilotAgent` wrapper. We **do** use this, but only for the **codegen sub-agent** (see §9 above). It is *not* the host runtime — the host runtime is a plain MAF `Agent` over a Foundry `gpt-5.x` deployment. See [AGENTS.md §3 invariant 12](../AGENTS.md) for the dual-runtime rationale.
