...
preamble naming the active project, e.g.

    [charter-agent-context: project_id=p-9a3f12 is_new=false]
...
def _strip_preamble_text(text: str) -> tuple[str, str | None, bool]:
    """Return `(stripped_text, project_id_or_None, is_new)`."""
    m = _PREAMBLE_RE.match(text or "")
    if not m:
        return text, None, False
    pid = m.group("pid")
    is_new = (m.group("new") or "false").lower() == "true"
    return text[m.end():], pid, is_new
...
def route_input_items(items: Any) -> Any:
    ...
    first text-bearing content, and side-effect `state.set_active_project_id`.
    ...
    if isinstance(items, str):
        new_text, pid, is_new = _strip_preamble_text(items)
        if pid:
            _apply(pid, is_new)
            return new_text
        return items
    ...
    for it in items:
        ...
        for blk in blocks:
            ...
            if not isinstance(text, str):
                continue
            new_text, pid, is_new = _strip_preamble_text(text)
            if pid is None:
                continue
            _apply(pid, is_new)
            if hasattr(blk, "text"):
